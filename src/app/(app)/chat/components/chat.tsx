...
import type { Chat, User, Message, AttachmentMetadata } from '@/lib/'; // Updated lib/types
...
// import { RequestDmDialog } from '../../profile/components/request-dm-dialog'; // REMOVED
...
export function Chat({ chat, loggedInUser, setMessages, highlightMessageId, isLoadingMore, hasMoreMessages, topMessageSentinelRef, scrollContainerRef, initialUnreadCount = 0 }: ChatProps) {
    ...
    // Removed dmRequests, sendDmRequest, unblockUser, and blockedUsers
    const {
        themeSettings,
        allUsers,
        leaveGroup,
        deleteGroup,
        forwardMessage,
        relationships, // We get this but pass it to ChatInput via context
    } = useAppContext();
    // --- END UPDATE ---

    const [editingMessage, setEditingMessage] = useState<{ id: number; content: string } | null>(null);
    const [replyingTo, setReplyingTo] = useState<Message | null>(null);
    // const [isRequestDmOpen, setIsRequestDmOpen] = useState(false); // REMOVED
                                        ...
                                        <EmojiPicker
                                            onEmojiClick={(emojiData: EmojiClickData) => {
                                                if (emojiData.isCustom) {
                                                    onCustomReact(emojiData.unified, message);
                                                } else {
    ...
    return (
        <div className="flex h-dvh flex-col">
            ...
            {/* All the obsolete props are now removed */}
            <ChatInput
                chat={chat}
